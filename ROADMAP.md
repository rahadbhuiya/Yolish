...
| **v2.32** | **`y.net.udp_send`'s `host` argument accepts a hostname literal now, closing the one gap the v2.20 changelog entry (native UDP's first release) explicitly flagged as not-yet-done: *"`udp_send`'s `host` is an IPv4 literal only, not yet a hostname."*
>
> Resolution is a new runtime routine, `__ys_net_udp_send_host`, not a change to `y.net.connect`'s own hostname resolver (`__ys_net_connect_host`). That function is proven — its own comment cites live verification against several CNAME-chained real hosts — and it's also large and delicate (a ~480-line hand-assembled routine covering `/etc/resolv.conf` discovery, building and sending the DNS query, and walking the answer records). Refactoring a shared "resolve" primitive out of it to reuse from this new UDP path was considered and rejected: that function's resolution and connection-attempt logic are genuinely interleaved, not separable without changing its behavior (it tries connecting to *each* A record as it's parsed, keeping whichever one succeeds first — see its own comment on why, verified live against a deliberately-blackholed first record), and touching proven, delicate, already-shipped code for the sake of a narrower feature risked a regression in the higher-value TCP path for no real gain. Duplicating the resolver-discovery/DNS-query/response-parsing portion — reused essentially byte-for-byte from `__ys_net_connect_host`, not re-transcribed from scratch — into a dedicated routine was the lower-risk choice, with `__ys_net_connect_host` itself left completely untouched.
>
> Past that shared portion, the new routine's answer-record loop is genuinely *simpler* than the one it's adapted from, not just copied: UDP's `sendto` on a resolved IP either succeeds or it doesn't — there's no connection to attempt, so nothing to retry against multiple records for. It takes the first A record found and stops, with none of the non-blocking-`connect`/`poll`/`SO_ERROR` machinery that exists in the TCP version solely to bound each connection attempt to a few seconds.
>
> `y.net.udp_send`'s own compile-time dispatch (in `compile_expr`) now checks the host literal's shape — same `is_ipv4_literal` check `y.net.connect` already uses — and calls whichever runtime routine fits, decided entirely at compile time; a hostname builds a DNS query via the same `build_dns_query` compile-time helper the TCP hostname path uses.
>
> One real mistake surfaced *while building this*, caught immediately by a syntax/brace-balance check rather than by testing: an early edit meant to append the new routine after `__ys_net_udp_send`'s closing brace instead replaced that function's final `syscall` instruction and epilogue outright, leaving it falling straight through into the new routine's code with no `ret` between them. Not a subtle bug — the file didn't compile at all, `gcc` immediately flagged corrupted-looking declarations several hundred lines later, right where the parser lost track of a brace that should have closed 6 lines earlier — but included here since it's the kind of mistake that "reads fine to a human skimming a large diff" and is exactly what makes actually building it (and having it fail loudly and immediately) worth more than reviewing it.
>
> Verified against a local stub DNS resolver standing in for a real one, in the same shell session as the compiled test binary (an earlier attempt across separate tool invocations silently lost the backgrounded stub process between them, which looked identical to a resolution failure until traced down to that): confirmed the query actually arrives at the resolver, the resolved address's UDP listener receives the exact bytes sent, and — separately — that pointing resolution at a resolver that never answers correctly times out and returns `-1` at the same ~3-second `SO_RCVTIMEO` bound the TCP hostname path already uses, rather than hanging forever. The existing IPv4-literal `udp_send` path, `__ys_net_connect_host`'s own TCP hostname resolution, the full interpreter/native regression suite, and `make windows` were all re-verified unaffected** |
| **v2.33** | **Two real gaps closed in native TLS/HTTP, plus a deliberate decision on a third that's worth recording as much as the two that got built.
>
> **Certificate verification.** `y.net.tls_connect` and `y.http.get_print`/`post_print`'s HTTPS path both trusted whatever certificate a server presented, full stop — no chain-of-trust check, no hostname match. Now both call `SSL_CTX_set_verify(SSL_VERIFY_PEER, NULL)`, set a TLS 1.2 floor, load the system's default CA trust store (`SSL_CTX_set_default_verify_paths`), and — for the hostname branch — `SSL_set1_host` so the presented certificate has to actually match the hostname being connected to, not just chain to a trusted root. This is the exact same combination the interpreter/VM `make tls` build already uses (`net_runtime.c`'s `ys_tls_ensure_ctx`), not a new approach invented for this pass. `SSL_CTX_set_min_proto_version` turned out not to be a real exported symbol in this OpenSSL build — only its underlying `SSL_CTX_ctrl`-based implementation is (`SSL_CTRL_SET_MIN_PROTO_VERSION`=123) — the exact same situation `SSL_set_tlsext_host_name`/`SSL_ctrl` already had two versions ago, just missed on a second function this time; caught immediately by a `symbol lookup error` at runtime on the very first test.
>
> **HTTP chunked-transfer-encoding decoding.** Previously, a chunked response's hex chunk-size lines and boundary CRLFs printed as literal text mixed into the body — visible in real testing against Cloudflare-fronted hosts as a stray `22f` (or similar) sitting right before the actual HTML. Decoding required restructuring how `y.http.*` reads a response at all: the existing read loop printed each `read()`/`SSL_read()` chunk immediately as it arrived, which can't work for chunked decoding since a chunk's own size line can itself be split across two separate reads — decoding needs the complete response in hand before any of it can be interpreted. The loop now accumulates into the same 4095-byte buffer `tls_recv_print` uses (reading into `rbuf+total` each iteration with capacity clamped to what's left, not always overwriting from offset 0) and only decodes/prints once, after the connection closes. A dedicated `emit_http_decode_and_print()` finds the header/body blank-line split, scans the header section for the substring `"chunked"`, and — if present — walks the chunk structure (parse a hex size, skip to the line's `\r\n`, print exactly that many bytes, skip the trailing `\r\n`, repeat until a zero-size chunk) instead of printing the buffer verbatim. Real trade-off, stated plainly rather than glossed over: a chunked response larger than 4095 bytes now truncates, where the old streaming approach could print an arbitrarily large *non-chunked* response in full — not a regression for chunked responses specifically (they were already unusable past the first read before this), but a real new limit for anything chunked and large.
>
> Building both of these surfaced the *same class* of mistake twice more — not the symbol issue above, a different one: a `str_replace` insertion landing in the middle of an existing function and deleting its own closing brace/tail in the process, leaving it falling straight through into whatever got inserted next with no `ret` between them. This happened once in this pass (during the chunked-decode restructure) on top of the one already logged in v2.32's entry — three times total across this native-networking stack now. Both of this pass's instances were caught immediately by a full-file brace-balance check (written to properly skip comments and string literals, since a naive character count isn't reliable enough on a file this size with this many string literals and comments) rather than by testing or by `gcc`'s own error output, which in one case pointed hundreds of lines away from the actual mistake. Worth calling out as a distinct, cheap, mechanical habit: run a brace-balance check on any large insertion into an existing function *before* reaching for a compile or a test binary — it would have caught all three instances in seconds rather than requiring a debugging pass each time.
>
> **macOS native networking was raised as a possible next step and deliberately not attempted.** Porting the raw-syscall layer needs more than swapping syscall numbers: BSD-derived `sockaddr_in`/`sockaddr_in6` put a length byte before the family field, unlike Linux's, so every hand-built sockaddr in this file — a good dozen call sites — would need auditing, and TLS/HTTP additionally depend on ELF-only dynamic-linking machinery with no Mach-O equivalent at all (roughly v2.27's whole milestone, again, for a different object format). None of that was the actual reason to stop, though: there's no macOS environment available anywhere in this project's toolchain to run a single line of the result on. Every real bug caught while building this entire native networking stack — this entry's two included — was caught by *running* the code against a real server or a full-file structural check, not by reading it; several looked entirely correct on review and only failed at runtime or at compile time. Code that can never be run isn't verified, it's just typed, and shipping an unrun macOS port would abandon the standard the rest of this stack was actually held to. Left for whenever a macOS environment to test against exists — see the DOCS.md native-compilation section for the same note in context.
>
> Verified: certificate verification against both a real self-signed pair (rejected, `-1`) and a real trusted host (accepted, handle returned) on both `tls_connect` and `get_print`; chunked decoding against a local server sending `"Hello"` and `" World"` as two separate chunks split across two separate TCP sends (correctly reassembled into `"Hello World"`, every marker stripped) and a real non-chunked response (unaffected, printed as before); full interpreter/native regression suite and `make windows` clean** |
...
- Native compilation followed in v2.25 (see that entry) — narrower
  ...
  there's no map or runtime string type natively. `udp_send`'s `host`
  was IPv4-literal only at first; hostname support followed in v2.32
  (see that entry).
